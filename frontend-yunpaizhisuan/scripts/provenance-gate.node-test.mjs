...
test('uses explicit release env over local Git values', () => {
  assert.deepEqual(
    resolveBuildMetadata({
      env: {
...
test('falls back to clean local Git values when explicit build inputs are absent', () => {
...
test('reports a dirty Git worktree without blocking a development build', () => {
...
test('falls back to unknown and reports dirty when Git is unavailable', () => {
  const resolved = resolveBuildMetadata({ env: {}, git: () => { throw new Error('missing'); }, now });
  assert.equal(resolved.commit, 'unknown');
  assert.equal(resolved.branch, 'unknown');
  assert.equal(resolved.dirty, true);
  assert.throws(() => assertReleaseMetadata(resolved), /commit/);
});

test('creates page metadata with commit, branch, time, and dirty state', () => {
...
test('release validation rejects a commit that is not a 40-character lowercase SHA', () => {
  ...
  for (const commit of ['abc', `${'c'.repeat(39)}Z`, `${'C'.repeat(40)}`, `${'c'.repeat(41)}`]) {
    assert.throws(() => assertReleaseMetadata({ ...valid, commit }), /commit/);
  }
});

test('release validation rejects a non-UTC buildTime', () => {
  const valid = { commit: 'c'.repeat(40), branch: 'release', buildTime: '2026-07-20T09:00:00.000Z', dirty: false };
  assert.throws(() => assertReleaseMetadata({ ...valid, buildTime: '2026-07-20T09:00:00+08:00' }), /UTC/);
  assert.throws(() => assertReleaseMetadata({ ...valid, buildTime: '2026-07-20T09:00:00' }), /UTC/);
});

test('release validation rejects unknown, empty, dirty, and placeholder values', () => {
  const valid = { commit: 'c'.repeat(40), branch: 'release', buildTime: '2026-07-20T09:00:00.000Z', dirty: false };
  ...
  assert.throws(() => assertReleaseMetadata({ ...valid, branch: 'changeme' }), /placeholder/);
  assert.throws(() => assertReleaseMetadata({ ...valid, branch: 'your_key' }), /placeholder/);
  assert.throws(() => assertReleaseMetadata({ ...valid, branch: '<placeholder>' }), /placeholder/);
});
