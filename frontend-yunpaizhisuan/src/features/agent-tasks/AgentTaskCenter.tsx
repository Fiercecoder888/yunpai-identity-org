...
function LocalGateNotificationCard({
  ...
  onDecision: (messageId: string, runId: string, decision: string, supplement?: Record<string, unknown>) => Promise<void>;
}) {
  ...
  const decide = async (decision: 'approve' | 'retry' | 'reject') => {
    ...
    setSubmitting(true);
    try {
      await onDecision(messageId, runId, decision, supplement);
    } catch (cause) {
  ...
  return <div className="local-gate-notification-card">
    ...
    <textarea aria-label="给 AI 的补充或追问" value={supplementText} onChange={(event) => setSupplementText(event.target.value)} placeholder={isM2DataGate ? '{"product_code":"P-100","bom_lines":[{"material_code":"MAT-1","quantity":1}]}' : '补充信息（可选）'} rows={2} disabled={submitting} />
    {error ? <div className="local-gate-notification-error" role="alert">{error}</div> : null}
    <Space wrap className="local-gate-notification-actions">
      {visibleActions.map((label) => { const kind = gateActionKind(label); return <Button key={label} size="small" danger={kind === 'reject'} type={kind === 'approve' ? 'primary' : 'default'} loading={submitting} disabled={submitting} onClick={() => void decide(kind)}>{label}</Button>; })}
    </Space>
  </div>;
}
...
export function AgentTaskCenter({ supervision = false }: { supervision?: boolean }) {
  ...
  useEffect(() => {
    ...
    notificationApi.warning({
      ...
      description: <LocalGateNotificationCard messageId={localGateMessage.id} runId={localGateEnvelope.runId} gate={localGateEnvelope.gate} onDecision={resumeGate} />,
      placement: 'bottomRight',
      duration: 0,
      className: 'local-gate-notification',
    });
  }, [localGateEnvelope, localGateMessage, localGateNotificationKey, notificationApi, resumeGate]);
