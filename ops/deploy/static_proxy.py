"""Small stdlib-only frontend server.

It serves the built Vite application and proxies /api requests to the local
Yunpai API.  Keeping this dependency-free is intentional: deployment hosts
may have Python but no Node runtime.
"""
...
def main() -> None:
    ...
    print(f"serving {os.getcwd()} on http://{args.host}:{args.port}, backend {args.backend_host}:{args.backend_port}", flush=True)
