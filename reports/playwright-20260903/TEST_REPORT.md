...
按用户确认，撤销右栏 80% 高度和自定义 scrollbar 外观，恢复初始版本的全高 `.progress-rail` 视觉（原背景、节点、进度线和默认滚动条）。仅保留外层高度约束，确保右栏从视口顶部到页面底部铺满。线上 1280x720 实测右栏 `top=0`、`bottom=720`、`height=720`，`scrollbar-color=auto`、`scrollbar-width=auto`。
