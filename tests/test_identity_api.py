...
def test_guidance_suggest_endpoint_no_persistence(client, tmp_path, store):
    """阶段① 红线：suggest 产出建议不落库（org/bindings 均无写入）。"""
    _seed_m0_workers(tmp_path)
    resp = client.post("/api/identity/guidance/suggest", headers=_ADMIN_HEADERS,
                       json={"tenant_id": "tenant-a"})
    assert resp.status_code == 200
    payload = resp.json()
    assert payload["persisted"] is False
    suggestions = {item["user_id"]: item for item in payload["plan"]["suggestions"]}
    assert suggestions["***********1234"]["suggested_roles"] == ["worker"]  # 仓管
    assert suggestions["***********9012"]["suggested_roles"] == ["team-leader"]  # 押出组长
    assert suggestions["***********5678"]["needs_review"] is True  # 办公室文员 → data-steward 待确认
    # 不落库红线：org 树与绑定表都为空
    assert store.org_tree(tenant_id="tenant-a") == []
    assert store.list_bindings(tenant_id="tenant-a") == []


def test_guidance_plan_draft_then_human_confirm_gate(client, tmp_path, store):
    """阶段④：draft 方案只写方案行；confirm 缺失拒绝；确认后绑定落库；重复应用拒绝。"""
    _seed_m0_workers(tmp_path)
    # 先派生组织树（apply 的 org 引用需要真实节点）
    derive = client.post("/api/identity/org/derive", headers=_ADMIN_HEADERS,
                         json={"tenant_id": "tenant-a"})
    assert derive.status_code == 200
    # 生成 draft 方案
    plan_resp = client.post("/api/identity/guidance/plan", headers=_ADMIN_HEADERS,
                            json={"tenant_id": "tenant-a"})
    assert plan_resp.status_code == 200
    plan_id = plan_resp.json()["plan_id"]
    assert plan_resp.json()["status"] == "draft"
    assert plan_resp.json()["bindings_written"] == 0
    # 红线：draft 阶段库里查不到任何绑定写入
    assert store.list_bindings(tenant_id="tenant-a") == []
    # 未确认应用 → 409，仍无绑定
    denied = client.post("/api/identity/guidance/apply", headers=_ADMIN_HEADERS,
                         json={"tenant_id": "tenant-a", "plan_id": plan_id, "confirm": False})
    assert denied.status_code == 409
    assert denied.json()["detail"]["code"] == "GUIDANCE_GATE_REJECTED"
    assert store.list_bindings(tenant_id="tenant-a") == []
    # 人工确认 → 绑定落库
    applied = client.post("/api/identity/guidance/apply", headers=_ADMIN_HEADERS,
                          json={"tenant_id": "tenant-a", "plan_id": plan_id, "confirm": True})
    assert applied.status_code == 200, applied.text
    assert applied.json()["status"] == "applied"
    bindings = {b["user_id"]: b for b in store.list_bindings(tenant_id="tenant-a")}
    assert set(bindings) == {"***********1234", "***********5678", "***********9012"}
    assert bindings["***********9012"]["role_codes"] == ["team-leader"]
    assert bindings["***********9012"]["org_id"] == "dept:押出"
    # 重复应用拒绝（幂等保护）
    again = client.post("/api/identity/guidance/apply", headers=_ADMIN_HEADERS,
                        json={"tenant_id": "tenant-a", "plan_id": plan_id, "confirm": True})
    assert again.status_code == 409


def test_guidance_apply_requires_admin(client, tmp_path, store):
    _seed_m0_workers(tmp_path)
    plan = client.post("/api/identity/guidance/plan", headers=_ADMIN_HEADERS,
                       json={"tenant_id": "tenant-a"})
    plan_id = plan.json()["plan_id"]
    resp = client.post("/api/identity/guidance/apply",
                       headers={"X-Actor-User": "worker-1", "X-Actor-Roles": "viewer-only"},
                       json={"tenant_id": "tenant-a", "plan_id": plan_id, "confirm": True})
    assert resp.status_code == 403
    assert store.list_bindings(tenant_id="tenant-a") == []
